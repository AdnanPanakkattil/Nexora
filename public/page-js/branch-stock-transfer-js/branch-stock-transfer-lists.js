$(document).ready(function () {
    ...
    var branchStockTransferTable = $("#branch_stock_transfer_table").DataTable({
        ...
        serverSide: true,
        ajax: {
            url: BASE_URL + "/branchstocktransfer/branch-stock-transfer-lists", // URL for the server-side processing
            data: function (d) {
                d.sourceBranch = $("#sourceBranch").val(); // send selected branch
                d.destinationBranch = $("#destinationBranch").val(); // send selected branch
                d.status = $("#status").val(); // send selected branch
        ...
        columns: [
            { data: "stockTransferId", name: "stockTransferId" },
                ...
                render: function (data, type, full, meta) {
                    ...
                    if (full.status == "on_processing") {
                        return (
                            '<a href="' +
                            detailsUrl +
                            '" class="btn btn-outline-primary waves-effect " style="width:100%;" ><i class="ti ti-report-medical"></i>Receive</a>'
                        );
                    } else {
                        return "";
                    }
                ...
                render: function (data, type, full, meta) {
                    ...
                    if (full.status === "on_processing") {
                        return (
                            ...
                            '">Details</a></li>' +
                            '<div class="dropdown-divider"></div>' +
                            '<li><a href="javascript:;" class="dropdown-item text-danger delete-stock-transfer" data-id="' +
                            deleteUrl +
                            '">Delete</a></li>' +
                            '<div class="dropdown-divider"></div>' +
    ...
    $("#stockTransferSearchBtn").on("click", function () {
    ...
    branchStockTransferTable.on("click", ".delete-stock-transfer", function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status === true) {
                            branchStockTransferTable.ajax.reload(null, false);
