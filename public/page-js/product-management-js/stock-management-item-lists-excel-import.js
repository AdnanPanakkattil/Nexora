$(document).ready(function () {
    ...
    $('#upload_excel_btn').on('click', function (e) {
        ...
        let stockManagementId = $('#stockManagementId').val();
        ...
        formData.append('stockManagementId', stockManagementId);
        formData.append('branchId', branchId);

        $("#loader-overlay").show();
$('#upload_excel_btn, #choose-file-btn, #excelImportBtn, #addItemStockModalBtn').prop('disabled', true);
    $('#upload_excel_btn').text('Processing...');

        // Sending AJAX POST request with the file
        $.ajax({
            url: BASE_URL + '/stock-management-item-lists-excel-import', // Change this to your upload route
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if (response.status == true) {
                    Swal.fire({
                        ...
                        localStorage.setItem("excelImportRunning", "true");
                        localStorage.setItem("excelImportStockId", stockManagementId);
                        window.location.href = BASE_URL + "/stock-management/" + stockManagementId;
            ...
            error: function (xhr, status, error) {
                ...
                alert('An error occurred while uploading the file.');
                 $('#upload_excel_btn, #choose-file-btn, #excelImportBtn, #addItemStockModalBtn').prop('disabled', false);
            $('#upload_excel_btn').text('Upload');
            }
        });
    });
