$(document).ready(function () {
    ...
    if ($("#laboratory_edit_id").val()) {
        initialPageLoad($("#laboratory_edit_id").val());
    }
...
$(document).on("click", "#saveButton", function (e) {
    e.preventDefault();
    var formData = new FormData($("#laboratory_form")[0]);
    $.ajax({
        url: $("#laboratory_form").attr("action"),
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function initialPageLoad(laboratoryEditId) {
    $.ajax({
        url: BASE_URL + "/edit-laboratory/" + laboratoryEditId,
        type: "GET",
        success: function (response) {
            if (response.status) {
