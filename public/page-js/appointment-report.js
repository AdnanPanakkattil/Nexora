$(document).ready(function () {
    ...
    var appointmentReportTable = $("#appointment_report_table").DataTable({
        ...
        serverSide: true,
        ajax: {
            url: BASE_URL + "/appointment-report",
            data: function (d) {
        ...
        columns: [
            ...
            { data: "financialCategory", name: "financialCategory" },
            ...
            { data: "reservationsType", name: "reservationsType" },
    ...
    appointmentReportTable.on("xhr.dt", function (e, settings, json) {
        if (json && json.summary) {
            $("#today_appointment").text(json.summary.today);
            $("#total_pending").text(json.summary.pending);
            $("#total_completed").text(json.summary.completed);
            $("#total_rejected").text(json.summary.rejected);
        }
    });
    ...
    appointmentReportTable.on("xhr.dt", function (e, settings, json) {
        if (json && json.summary) {
            $("#today_appointment").text(json.summary.today_appointment || 0);
            $("#pending").text(json.summary.pending || 0);
            $("#completed").text(json.summary.completed || 0);
            $("#rejected").text(json.summary.rejected || 0);
        }
    });

    $(
        "#clinicId, #employeeId, #createdEmployeeId, #serviceCategoryId, #status, #reservationsType, #serviceId, #clientId",
    ).on("change", function () {
        appointmentReportTable.ajax.reload();
    });
