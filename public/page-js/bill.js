...
$(document).ready(function () {
    ...
    function calculateTotal() {
        ...
        $("tbody#service_div tr").each(function () {
            ...
            // Only one discount type is active at a time
            var discountAmount =
                discountPercent > 0
        ...
        $(".invoice-item-price").val(grandTotal.toFixed(2));
        ...
        function redistributePaymentAmounts() {
            ...
            var amountPerMethod =
                Math.floor((grandTotal / numberOfSelected) * 100) / 100; // floor, not round
            var remainder = parseFloat(
                (grandTotal - amountPerMethod * numberOfSelected).toFixed(2),
            );

            totalvalue = grandTotal; // use exact grandTotal

            var index = 0;
            selectedCheckboxes.each(function () {
                var $parentDiv = $(this).closest("div.border-bottom");
                var isLast = index === numberOfSelected - 1;
                var amount = isLast
                    ? parseFloat((amountPerMethod + remainder).toFixed(2))
                    : amountPerMethod;
                $parentDiv.find(".amount-input").val(amount.toFixed(2));
                index++;
            });
            $(".amount-input:visible")
                ...
                .val(
                    parseFloat(
                        (amountPerMethod + remainder).toFixed(2),
                    ).toFixed(2),
                );
        }
    }
    ...
    function updateInvoiceCalculations() {
        ...
        $("#service_div tr").each(function () {
            ...
            const vatAmount = roundToTwo(
                ((rowTotal - discountAmount) * vatPercent) / 100,
            );
    ...
    $(".payment-checkbox").change(function () {
        ...
        var amountPerMethod =
            numberOfSelected > 0
                ? Math.floor((grandTotal / numberOfSelected) * 100) / 100
                : 0;
        var remainder = parseFloat(
            (grandTotal - amountPerMethod * numberOfSelected).toFixed(2),
        );
    ...
    $("#serviceId")
        ...
        .select2({
            ...
            ajax: {
                url: BASE_URL + "/bill/search-service",
                dataType: "json",
                delay: 250,
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status === true) {
                        ...
                        const test = $("#serviceId").val();
                        var nationality = $("#selectnationality").val();
                        console.log("Nationality:", nationality);
                        var vatEnabled = nationality && nationality !== "0";
                        console.log("VAT Enabled:", vatEnabled);
    ...
    function saveInvoice(buttonid) {
        ...
        $("#service_div tr").each(function () {
            ...
            // alert(row.find('.selected-service-name input').val());
                ...
                if (response.status === true) {
                    Swal.fire({
                        ...
                        window.location.href =
                            BASE_URL + "/invoice/preview/" + response.data;
                    });
                } else {
                    Swal.fire({
                        icon: "error",
    ...
    $("#draftbtn").click(function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // User clicked "Yes", so save the draft
                saveInvoice($(this).attr("id"));
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                // User clicked "Cancel", do nothing
                Swal.fire({
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === true) {
                    ...
                    // You can now use serviceOrderMasterId to preview the invoice
                    console.log(
...
// Prevent typing negative sign (-) or 'e' (scientific notation)
// $(document).on("keydown", ".discount-percent",
//  function (e) {
//     if (e.key === "-" || e.key === "e") {
//         e.preventDefault();
//     }
...
// Proper validation: allow decimal up to 2 digits, allow 10.10
$(document).on("input", ".discount-percent", function () {
    ...
    // Allow ending dot while typing (e.g., "10.")
    if (val.endsWith(".")) {
        $(this).val(val);
        return;
    }

    // Limit decimal to 2 digits (preserve trailing zero)
    if (parts[1] && parts[1].length > 2) {
        parts[1] = parts[1].substring(0, 2);
    }
    val = parts.join(".");

    // Remove leading zeros unless "0.xx"
    if (val.length > 1 && val.startsWith("0") && !val.startsWith("0.")) {
        val = val.replace(/^0+/, "") || "0";
    }

    // Clamp range 0–100 without losing decimals
    let num = parseFloat(val);
    if (!isNaN(num)) {
        if (num > 100) val = "100"; // no decimals allowed beyond 100
    }
...
$(document).on("input", ".flat", function () {
    let row = $(this).closest("tr"); // the row of this item

    let qty = parseFloat(row.find(".quantity").val()) || 0;
    let cost = parseFloat(row.find(".cost").val()) || 0;

    let maxAllowed = qty * cost; // actual amount
    let discount = parseFloat($(this).val()) || 0;

    // Prevent discount > (qty × cost)
    if (discount > maxAllowed) {
        $(this).val(maxAllowed.toFixed(2)); // Auto-correct
    }
});
...
$(document).on("change", "#branchSelect", function () {
    ...
    if (branchId !== "") {
        $("#loader-overlay").show();
        $.ajax({
            url: BASE_URL + "/get-branch-details/" + branchId,
            type: "GET",
            success: function (response) {
                $("#loader-overlay").hide();
                if (response.status === true) {
