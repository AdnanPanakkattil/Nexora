$(document).ready(function () {
    ...
    $('#choose-file-import-btn').on('click', function (e) {
        ...
        $('#sbsblockexcelfileupload').click();
    });

    $('#sbsblockexcelfileupload').on('change', function () {
        let fileName = this.files.length > 0 ? this.files[0].name : "No file chosen";
        $('#file-name').text(fileName);
        $('#FileValidationMsg').addClass('d-none'); // Hide validation message
    });

    $('#sbs_block_upload_excel_file_btn').on('click', function (e) {
        e.preventDefault(); 

        let fileInput = $('#sbsblockexcelfileupload')[0];
        if (fileInput.files.length === 0) {
            $('#SBSBlockExcelFileValidationMsg').removeClass('d-none').text('Please select appropriate Excel file before uploading.');
            return;
        }
        ...
        $.ajax({
            url: BASE_URL + '/sbs-block-excel-import', 
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
