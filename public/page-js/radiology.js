$(document).ready(function () {
    ...
    if ($("#radiologist_edit_id").val()) {
        initialPageLoad($("#radiologist_edit_id").val());
    }
...
$(document).on("click", "#saveButton", function (e) {
    ...
    var formData = new FormData($("#radiology_form")[0]);
    $.ajax({
        url: $("#radiology_form").attr("action"),
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function initialPageLoad(radiologistEditId) {
    $.ajax({
        url: BASE_URL + "/edit-radiology/" + radiologistEditId,
        type: "GET",
        success: function (response) {
            if (response.status) {
