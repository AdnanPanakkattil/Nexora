$(document).ready(function () {
    ...
    if ($("#receptionist_edit_id").val()) {
        initialPageLoad($("#receptionist_edit_id").val());
    }
...
$(document).on("click", "#saveButton", function (e) {
    ...
    var formData = new FormData($("#receptionist_form")[0]);
    $("#page-loader").show();
    $.ajax({
        url: $("#receptionist_form").attr("action"),
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function initialPageLoad(receptionistEditId) {
    $.ajax({
        url: BASE_URL + "/edit-receptionist/" + receptionistEditId,
        type: "GET",
        success: function (response) {
            if (response.status) {
