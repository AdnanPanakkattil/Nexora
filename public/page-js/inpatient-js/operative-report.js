...
Dropzone.options.xrayFileUploadForm = {
    ...
    <div class="dz-preview dz-file-preview">
        <div class="subimage">
            ...
            <div class="dz-actions mt-2 text-center">
                <button class="btn btn-sm btn-label-secondary dz-view" type="button" onclick="viewOperativeReportFile(event)" data-file-url="" disabled>View</button>
                <button class="btn btn-sm btn-label-danger dz-delete" type="button" onclick="deleteOperativeReportFile(event)">Delete</button>
            </div>
    ...
    init: function () {
        this.on("success", function (file, response) {
            ...
            formData.append("operative_report_file", file);
            formData.append("operative_report_file", file.name);
            ...
            fetch("/inpatient-upload-operative-report-file", {
                method: "POST",
                body: formData,
            })
        ...
        window.deleteOperativeReportFile = function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`/inpatient-delete-operative-report-file/${fileId}`, {
                        method: "DELETE",
                        headers: {
...
function viewOperativeReportFile(event) {
    const fileId = event.target.getAttribute("data-file-id");
    if (fileId) {
        fetch("/inpatient-view-operative-report-file/" + fileId, {
            method: "GET",
        })
            .then((response) => response.json())
            .then((data) => {
