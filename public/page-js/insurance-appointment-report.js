$(document).ready(function () {
    ...
    $("#insurance-appointmrent_report_sub_menu").addClass("active");
...
$("#serviceCategoryId")
    ...
    var appointmentReportTable = $("#insurance-appointment_report_table").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: BASE_URL + "/insurance-appointment-report", // URL for the server-side processing
            data: function (d) {
        ...
        columns: [
            { data: "reservationId", name: "reservationId" },
            {
                ...
                render: function (data, type, full, meta) {
                    var detailsUrl = BASE_URL + "/medicalrecord-details/" + full.reservationId;
                    console.log(detailsUrl);
                    return (

                        '<a href="' +
                        detailsUrl +
                        '" class="btn btn-outline-primary waves-effect " style="width:100%;" ><i class="ti ti-report-medical"></i>' + full.mrId + '</a>'
                    );
                },
            },
    ...
    $('#clinicId, #employeeId, #createdEmployeeId, #status, #reservationsType, #serviceId, #clientId').on('change', function () {
        appointmentReportTable.ajax.reload();
    });
    ...
    appointmentReportTable.on("click", ".item-details", function () {
        var editUrl = $(this).data("id");
        var redirectUrl = "/detail-of-appointment-show/" + editUrl; // Adjust to match your route
    
        window.location.href = redirectUrl; // Redirect to the details page
    });
