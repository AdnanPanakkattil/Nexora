$(document).ready(function () {
    ...
    if ($("#pharmacy_edit_id").val()) {
        initialPageLoad($("#pharmacy_edit_id").val());
    }
...
$(document).on("click", "#saveButton", function (e) {
    ...
    var formData = new FormData($("#pharmacy_form")[0]);
    $.ajax({
        url: $("#pharmacy_form").attr("action"),
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function initialPageLoad(pharmacistEditId) {
    $.ajax({
        url: BASE_URL + "/edit-pharmacy/" + pharmacistEditId,
        type: "GET",
        success: function (response) {
            if (response.status) {
